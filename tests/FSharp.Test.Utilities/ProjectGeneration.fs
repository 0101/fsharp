...
module ProjectOperations =
    ...
    let checkFileWithIncrementalBuilder fileId (project: SyntheticProject) (checker: FSharpChecker) =
        let p, file = project.FindInAllProjects fileId
        let contents = renderSourceFile p file
        let absFileName = getFilePath p file

        checker.ParseAndCheckFileInProject(
            absFileName,
            0,
            SourceText.ofString contents,
            p.GetProjectOptions checker
        )
    ...
    let getSourceText (project: SyntheticProject) (filePath: string) =
        if filePath.EndsWith(".fsi") then
            ...
            let containingProject, source = project.FindInAllProjectsByPath implFilePath
            match source.SignatureFile with
            | No -> failwith $"{implFilePath} does not have a signature file"
            | Custom _ -> renderCustomSignatureFile containingProject source
            | AutoGenerated ->
            ...
            filePath
            |> project.FindInAllProjectsByPath
            |> (fun (p, f) -> renderSourceFile p f)
    ...
    let checkFileWithTransparentCompiler fileId (project: SyntheticProject) (checker: FSharpChecker) =
        async {
            ...
            let! projectSnapshot = FSharpProjectSnapshot.FromOptions(options, getFileSnapshot p)
